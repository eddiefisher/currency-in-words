...
module CurrencyInWords
  ...
  class RuTexterizer
    ...
    # :nodoc: all
    A = %w(ноль один два три четыре пять шесть семь восемь девять)
    B = %w(десять одиннадцать двенадцать тринадцать четырнадцать пятнадцать шестнадцать семнадцать восемнадцать девятнадцать)
    C = [nil,nil,'двадцать','тридцать','сорок','пятьдесят','шестьдесят','семдесят', 'восемдесят','девяноста']
    D = [nil,'тысяча','миллион','миллиард','триллион','квадраллион','квантиллион','секстиллион','септиллион','октиллион']
    ...
    def under_1000 number
      q,r = number.divmod 100
      arr = ([A[q]] << 'тысяча' + (' ' unless @skip_and || r.zero?).to_s) if q > 0
      r.zero? ? arr : arr.to_a << under_100(r)
    end
